#pragma once
#ifndef _JOSRT_arena_allocator_ALLOCATOR_H
#define _JOSRT_arena_allocator_ALLOCATOR_H
...
_JOSRT_API_FUNC arena_allocator_t* arena_allocator_create(void* mem, size_t size);
_JOSRT_API_FUNC void* arena_allocator_alloc(arena_allocator_t* arena, size_t size);
_JOSRT_API_FUNC void arena_allocator_free(arena_allocator_t* arena, void* block);
_JOSRT_API_FUNC void* arena_allocator_realloc(arena_allocator_t* arena, void* block, size_t size);

#if defined(_JOSRT_IMPLEMENT_ALLOCATORS) && !defined(_JOSRT_arena_allocator_ALLOCATOR_IMPLEMENTED)
#define _JOSRT_arena_allocator_ALLOCATOR_IMPLEMENTED

//NOTE: +7 because we align all allocations to nearest 8 bytes
#define _JOSRT_arena_allocator_ALLOC_OVERHEAD (sizeof(vmem_block_head_t) + sizeof(vmem_block_tail_t) + 7)
#define _JOSRT_VMEM_ABS_BLOCK_SIZE(size) ((size) & ~kVmemBlockFree)
#define _JOSRT_VMEM_BLOCK_MIN_SIZE _JOSRT_arena_allocator_ALLOC_OVERHEAD + 16
#define _JOSRT_arena_allocator_MIN_SIZE (sizeof(arena_allocator_t) + _JOSRT_arena_allocator_ALLOC_OVERHEAD)

_JOSRT_INLINE_FUNC size_t arena_allocator_available(arena_allocator_t* arena) {
    return arena->_size - _JOSRT_arena_allocator_ALLOC_OVERHEAD;
}

_JOSRT_INLINE_FUNC vmem_block_tail_t* _vmem_tail_from_head(const vmem_block_head_t* head)
{	
	return (vmem_block_tail_t*)((char*)(head+1) + (_JOSRT_VMEM_ABS_BLOCK_SIZE(head->_size)) - (_JOSRT_arena_allocator_ALLOC_OVERHEAD));
}

_JOSRT_INLINE_FUNC vmem_block_head_t* _vmem_head_from_tail(const vmem_block_tail_t* tail)
{	
	return (vmem_block_head_t*)((char*)(tail) - (_JOSRT_VMEM_ABS_BLOCK_SIZE(tail->_size) + sizeof(vmem_block_head_t)));
}

_JOSRT_INLINE_FUNC bool _vmem_block_is_free(size_t blockSize)
{
	return ((blockSize & kVmemBlockFree)==kVmemBlockFree);
}

_JOSRT_INLINE_FUNC size_t _vmem_tail_size(const vmem_block_head_t* head)
{
	return ((head->_size - (_JOSRT_arena_allocator_ALLOC_OVERHEAD)));
}

_JOSRT_INLINE_FUNC size_t _vmem_tail_free_size(const vmem_block_head_t* head)
{
	return ((head->_size - (_JOSRT_arena_allocator_ALLOC_OVERHEAD)) | kVmemBlockFree);
}

_JOSRT_INLINE_FUNC void _arena_allocator_block_insert_as_free(arena_allocator_t* arena, vmem_block_head_t* new_block)
{
	...
	else
	{
		...
		if(_JOSRT_VMEM_ABS_BLOCK_SIZE(new_block->_size) <= _JOSRT_VMEM_ABS_BLOCK_SIZE(free->_size))
		{
			arena->_free_head = new_block;
			new_block->_links[0] = 0;
...
_JOSRT_INLINE_FUNC void _arena_allocator_disconnect(arena_allocator_t* arena, vmem_block_head_t* block)
{
...
_JOSRT_API_FUNC arena_allocator_t*   arena_allocator_create(void* mem, size_t size)
{
    if(!mem || size <= _JOSRT_arena_allocator_MIN_SIZE)
    {
        return 0;
    }	
...
_JOSRT_API_FUNC void* arena_allocator_alloc(arena_allocator_t* arena, size_t size)
{
	...
	size = (size < _JOSRT_VMEM_BLOCK_MIN_SIZE) ? _JOSRT_VMEM_BLOCK_MIN_SIZE : (size+7)&~7;

	if(!arena || arena->_size<size+_JOSRT_arena_allocator_ALLOC_OVERHEAD )
	{
        return 0;
	}
    ...
    while(free)
    {
        if(_JOSRT_VMEM_ABS_BLOCK_SIZE(free->_size) >= (size+_JOSRT_arena_allocator_ALLOC_OVERHEAD) )
            break;
        free = free->_links[1];
    }
    ...
    if(free)
    {
        ...
        size_t org_size = _JOSRT_VMEM_ABS_BLOCK_SIZE(free->_size);		
        free->_size = (size+_JOSRT_arena_allocator_ALLOC_OVERHEAD);
		if(org_size - free->_size < _JOSRT_VMEM_BLOCK_MIN_SIZE)
		{
		...
		org_size -= free->_size;
        if(org_size >= _JOSRT_VMEM_BLOCK_MIN_SIZE)
        {
            vmem_block_head_t* new_head = (vmem_block_head_t*)_JOSRT_ALIGN(new_tail+1, kAllocAlign_8);
...
_JOSRT_API_FUNC void* arena_allocator_realloc(arena_allocator_t* arena, void* block, size_t size) {
	...
	if (_JOSRT_VMEM_ABS_BLOCK_SIZE(head->_size) >= size) {
	...
	if (new_block) {
		memcpy(new_block, block, _JOSRT_VMEM_ABS_BLOCK_SIZE(head->_size));
	}
...
_JOSRT_API_FUNC void arena_allocator_free(arena_allocator_t* arena, void* block)
{
    ...
    {
        const size_t prev_size = _JOSRT_VMEM_ABS_BLOCK_SIZE(next_head->_size);
...
#endif // _JOSRT_arena_allocator_ALLOCATOR_IMPLEMENTED

#endif // _JOSRT_arena_allocator_ALLOCATOR_H
