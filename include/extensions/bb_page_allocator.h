#ifndef _JOSRT_BB_PAGE_ALLOCATOR_H
#define _JOSRT_BB_PAGE_ALLOCATOR_H
...
_JOSRT_API_FUNC void bb_page_allocator_create(bb_page_allocator_t* palloc, void* pool_base, size_t pool_num_pages, static_allocation_policy_t* static_allocator);
_JOSRT_API_FUNC void* bb_page_allocator_allocate(bb_page_allocator_t* allocator, size_t page_count);
_JOSRT_API_FUNC void bb_page_allocator_free(bb_page_allocator_t* allocator, void* ptr, size_t page_count);

#if defined(_JOSRT_IMPLEMENT_ALLOCATORS) && !defined(_JOSRT_BB_PAGE_ALLOCATOR_IMPLEMENTED)
#define _JOSRT_BB_PAGE_ALLOCATOR_IMPLEMENTED

// define this prior to including with IMPLEMENT to change the allocation page size
#ifndef _JOSRT_BB_PAGE_ALLOCATOR_PAGE_SIZE
	#define _JOSRT_BB_PAGE_ALLOCATOR_PAGE_SIZE kAllocAlign_4k
#endif
...
} __packed _bb_page_block_header_t;
...
_JOSRT_API_FUNC void bb_page_allocator_create(bb_page_allocator_t* palloc, void* pool_base, size_t pool_num_pages, static_allocation_policy_t* static_allocator) {
	...
	assert(palloc);
	assert(pool_num_pages > 2);

	uintptr_t aligned_pool_base = _JOSRT_ALIGN(pool_base, _JOSRT_BB_PAGE_ALLOCATOR_PAGE_SIZE);
	...
	if (static_allocator) {
		...
		aligned_pool_base += (uintptr_t)_JOSRT_BB_PAGE_ALLOCATOR_PAGE_SIZE;
	}
	...
	for (int z = (int)n; z >= 0; --z) {
		if (pool_num_pages < pages) {
			...
			aligned_pool_base += (uintptr_t)_JOSRT_BB_PAGE_ALLOCATOR_PAGE_SIZE * pages;
...
_JOSRT_INLINE_FUNC void _bb_page_allocator_insert_pages(bb_page_allocator_t* allocator, size_t order, uintptr_t page_block, uintptr_t prev_page_block) {
...
_JOSRT_API_FUNC void* bb_page_allocator_allocate(bb_page_allocator_t* allocator, size_t page_count) {
	...
	uintptr_t realloc_page_ptr = prev_page_ptr + (page_count * _JOSRT_BB_PAGE_ALLOCATOR_PAGE_SIZE);
	...
	for (int z = (int)order; z >= 0 && pages_to_relocate; z--) {
		if (pages_in_zone <= pages_to_relocate) {
			...
			realloc_page_ptr += (pages_in_zone * _JOSRT_BB_PAGE_ALLOCATOR_PAGE_SIZE);
		}
		pages_in_zone >>= 1;
	}
	return ptr;
...
_JOSRT_API_FUNC void bb_page_allocator_free(bb_page_allocator_t* allocator, void* ptr, size_t page_count) {
	...
	size_t order = (size_t)_BB_PAGE_ALLOC_ORDER_HI(page_count);
	assert(order <= allocator->_max_order);
...
#endif // _JOSRT_BB_PAGE_ALLOCATOR_H
