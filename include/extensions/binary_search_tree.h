...
_JOSRT_API_FUNC void binary_search_tree_destroy(binary_search_tree_t* tree);

_JOSRT_API_FUNC _bsearch_tree_node_t* _binary_search_tree_insert_impl(binary_search_tree_t* tree, _bsearch_tree_node_t* parent, uintptr_t key, void* value);
/*
* Insert the given key[value] into the tree if it does not already exist
* Returns true if the node was inserted
*/
_JOSRT_INLINE_FUNC bool binary_search_tree_insert(binary_search_tree_t* tree, uintptr_t key, void* value) {
...
*/
_JOSRT_INLINE_FUNC bool binary_search_tree_contains(binary_search_tree_t* tree, uintptr_t key) {
	return _binary_search_tree_contains_impl(tree->_root, key);
}

_JOSRT_INLINE_FUNC bool _binary_search_tree_find_impl(binary_search_tree_t* tree, _bsearch_tree_node_t* node, uintptr_t key, void** out_value);
/*
* Returns true if the given key exists and will optionally return the value pointer (if it exists)
*/
_JOSRT_INLINE_FUNC bool binary_search_tree_find(binary_search_tree_t* tree, uintptr_t key, void** out_value) {
...
_JOSRT_API_FUNC void _binary_search_tree_sorted_traverse_impl(binary_search_tree_t* tree, _bsearch_tree_node_t* node, binary_search_tree_node_visitor_t visitor);
/*
...
_JOSRT_INLINE_FUNC int _binary_search_tree_height_delta(_bsearch_tree_node_t* node) {
	int lh = node->_left ? node->_left->_height : 0;
	int rh = node->_right ? node->_right->_height : 0;
	return lh - rh;
}

_JOSRT_INLINE_FUNC void _binary_search_tree_recalc_height(_bsearch_tree_node_t* node) {
	int lh = node->_left ? node->_left->_height : 0;
	int rh = node->_right ? node->_right->_height : 0;
...
_JOSRT_INLINE_FUNC void _binary_search_tree_node_destroy_impl(binary_search_tree_t* tree, _bsearch_tree_node_t* node) {
	// depth first delete
	if (node->_left) {
