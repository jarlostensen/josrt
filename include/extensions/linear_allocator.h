...
_JOSRT_API_FUNC linear_allocator_t* linear_allocator_create(void* memory, size_t size);
_JOSRT_API_FUNC void* linear_allocator_alloc(linear_allocator_t* linalloc, size_t size);
_JOSRT_INLINE_FUNC void linear_allocator_clear(linear_allocator_t* linalloc) {
    linalloc->_ptr = (char*)((char*)linalloc->_begin + sizeof(linear_allocator_t));
}

_JOSRT_INLINE_FUNC size_t linear_allocator_available(linear_allocator_t* linalloc) {
    return (size_t)linalloc->_end - (size_t)linalloc->_ptr;
}

#if defined(_JOSRT_IMPLEMENT_ALLOCATORS) && !defined(_JOSRT_LINEAR_ALLOCATOR_IMPLEMENTED)
#define _JOSRT_LINEAR_ALLOCATOR_IMPLEMENTED
